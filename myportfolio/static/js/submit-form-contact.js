...
(function(){
	...
	form.addEventListener('submit', async function(e){
		...
		try{
			...
			const endpoint = 'https://endpoint-pchg.onrender.com';
			...
			if((res.status === 405 || res.status === 404 || res.status === 0) && !endpoint.startsWith('https://endpoint-pchg.onrender.com')){
					const fallback = 'https://endpoint-pchg.onrender.com/contact-endpoint/';
				try{
			...
			msg.innerHTML = `<div style="padding:12px;border-radius:8px;background:rgba(255,20,60,0.06);color:#ffb3b3">Network error: ${err.message}</div>`;
		} finally{
